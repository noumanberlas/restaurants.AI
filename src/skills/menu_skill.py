...
- Only delete an item if explicitly asked to remove it permanently.
- Always confirm with the user before adding, updating, or removing any item.

WORKFLOW
1. Use get_menu to check what already exists before adding new items.
2. When adding, ask for name, price, and category if not provided.
3. When marking unavailable, tell the user the item will be hidden from customers.
"""
